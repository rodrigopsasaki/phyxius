...
import { generateEnvExample } from "./parsers/env.js";
...
export function createConfig<T>(schema: Validator<T>, options: ConfigOptions): ConfigInstance<T> {
  ...
  if (environment !== undefined) {
    initialMetadata.environment = environment;
  }
  ...
  if (initialLoad._tag === "Ok") {
    const loadedMetadata: ConfigMetadata = {
      loadedAt: clock.now(),
      sources,
      watchEnabled: watch,
      reloadCount: 0,
    ...
    if (environment !== undefined) {
      loadedMetadata.environment = environment;
    }
  ...
  } else if (initialLoad.error !== undefined) {
    const loadError = initialLoad.error;
